...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/code/" && method === "GET") {
      ...
      const sizeRaw = parseInt(url.searchParams.get("size") || "50", 10) || 50;
      const size = Math.min(200, Math.max(1, sizeRaw));

      // 先只取 key（超过 1000 条时用游标继续翻），避免 list 默认上限漏数据
      const keyNames = [];
      let cursor;
      do {
        const listed = await kv.list({ cursor });
        for (const k of listed.keys) keyNames.push(k.name);
      ...
      // 时间倒序；时间相同时按 id 兜底排序，保证分页结果稳定（不会重复/漏项）
      all.sort((a, b) => (new Date(b.time).getTime() - new Date(a.time).getTime()) || String(b.id).localeCompare(String(a.id)));
      ...
      return new Response(JSON.stringify({ items, total, page: currentPage, size, totalPages }), {
...
<script>
    ...
    function totalPages() { return Math.max(1, Math.ceil(totalRecords / PAGE_SIZE)); }
    function render() {
        const tbody = document.getElementById('tableBody');
        if (pageItems.length === 0) { tbody.innerHTML = '<tr><td colspan="3" class="empty-state">✨ 暂无验证码，等待接收...</td></tr>'; }
        else {
    ...
    function renderPagination() {
        const tp = totalPages();
        document.getElementById('pageInfo').innerHTML = '第 <b>' + currentPage + '</b> / ' + tp + ' 页，共 <b>' + totalRecords + '</b> 条';
        const wrap = document.getElementById('pageControls');
        if (tp <= 1) { wrap.innerHTML = ''; return; }
        const mk = (label, page, isActive) => { const b = document.createElement('button'); b.className = 'page-btn' + (isActive ? ' active' : ''); b.textContent = label; b.onclick = () => loadData(page); return b; };
        wrap.innerHTML = '';
        const prevBtn = document.createElement('button'); prevBtn.className = 'page-btn'; prevBtn.textContent = '‹ 上一页'; prevBtn.disabled = currentPage <= 1; if (currentPage > 1) prevBtn.onclick = () => loadData(currentPage - 1); wrap.appendChild(prevBtn);
        ...
        const nextBtn = document.createElement('button'); nextBtn.className = 'page-btn'; nextBtn.textContent = '下一页 ›'; nextBtn.disabled = currentPage >= tp; if (currentPage < tp) nextBtn.onclick = () => loadData(currentPage + 1); wrap.appendChild(nextBtn);
    }
    function setJumpInput() { document.getElementById('jumpPage').value = currentPage; }
    function handleJump() { let p = parseInt(document.getElementById('jumpPage').value, 10); if (isNaN(p)) p = 1; p = Math.max(1, Math.min(totalPages(), p)); setJumpInput(); loadData(p); }
    async function loadData(page) {
        const tbody = document.getElementById('tableBody');
        tbody.innerHTML = '<tr><td colspan="3" class="loading">⌛ 加载中...</td></tr>';
        const target = Math.max(1, parseInt(page, 10) || 1);
        try {
            const resp = await fetch('/code/?page=' + target + '&size=' + PAGE_SIZE);
    ...
    async function deleteRecord(id) {
        try {
            ...
            if (resp.ok) { loadData(currentPage); } else { alert('删除失败，请重试'); }
